...
String names[],sexs[],courss[],dat[],roll[],name[],sex[],course[],sem[],sts[];
String croll,cref;
...
try{
	...
	rsrefarray=stmtrefarray.executeQuery("select * from vet_tblstuAttend");
	i=0;
	while(rsrefarray.next()){
		 dat[i]=rsrefarray.getString("date").trim();
		 roll[i]=rsrefarray.getString("roll_no").trim();
		 name[i]=rsrefarray.getString("name");
		 sex[i]=rsrefarray.getString("sex").trim();
		 course[i]=rsrefarray.getString("course");
		 sem[i]=rsrefarray.getString("semester").trim();
		 sts[i]=rsrefarray.getString("ststus").trim();
...
function showstu(){
	window.location="vet_stuAttendc.jsp";
}
</script>
...
<form name="f1" action="vet_stuAttend_finds.jsp" method="get">
...
try{
	...
	rsref=stmtfillref.executeQuery("select * from vet_tblstuAttend");
	while(rsref.next()){
