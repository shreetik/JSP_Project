...
<%!
Connection conn;
Statement stmtfillroll,stmtfillarray,stmtfillref,stmtrefarray,stmtfillslip,stmtsliparray;
ResultSet rsroll,rsarray,rsref,rsrefarray,rsfillslip,rsslip;
String sdate[],sref[],sdat[],sroll[],sname[],ssex[],scourse[],ssem[],swarning[],sdis[];
String names[],sexs[],courses[],dat[],roll[],name[],sex[],course[],sem[],warning[];
String croll,cref,cslip;
int i;
...
sname=new String [30];
ssex=new String [30];
scourse=new String [30];
ssem=new String [30];
swarning=new String [30];
sdis=new String [30];

doconnect();
try{
	stmtsliparray=conn.createStatement();
	rsslip=stmtsliparray.executeQuery("select * from vet_tbldisaction");
	i=0;
	while(rsslip.next()){
		 sdate[i]=rsslip.getString("date_c").trim();
		 sref[i]=rsslip.getString("ref_no").trim();
		 sdat[i]=rsslip.getString("date").trim();
		 sroll[i]=rsslip.getString("roll_no").trim();
		 sname[i]=rsslip.getString("name");
		 ssex[i]=rsslip.getString("sex").trim();
		 scourse[i]=rsslip.getString("course");
		 ssem[i]=rsslip.getString("semester").trim();
		 swarning[i]=rsslip.getString("warning_for");
		 sdis[i]=rsslip.getString("dis_action");
...
var jsref=new Array();
var jsdat=new Array();
var jsroll=new Array();
var jsname=new Array();
var jssex=new Array();
var jscourse=new Array();
var jssem=new Array();
var jswarning=new Array();
var jsdis=new Array();
...
jsroll[j]= "<%=sroll[i]%>";	
 jsname[j]= "<%=sname[i]%>";
 jssex[j]= "<%=ssex[i]%>";
 jscourse[j]= "<%=scourse[i]%>";	
 jssem[j]= "<%=ssem[i]%>";	
 jswarning[j]= "<%=swarning[i]%>";	
 jsdis[j]= "<%=sdis[i]%>";	
...
%>
  k=document.f1.cmbslip.selectedIndex;
  
  document.f1.txtdat.value=jsdate[k-1];
 document.f1.cmbref.value=jsref[k-1];
 document.f1.txtdate.value=jsdat[k-1];
 document.f1.cmbroll.value=jsroll[k-1];
 document.f1.txtname.value=jsname[k-1];
 document.f1.cmbsex.value=jssex[k-1];
  document.f1.txtcrs.value=jscourse[k-1];
 document.f1.cmbsem.value=jssem[k-1];
 document.f1.txtwar.value=jswarning[k-1];
 document.f1.txtdis.value=jsdis[k-1];
}
...
<form name="f1" action="vet_disaction_finds.jsp" method="get">
<table border="0" cellspacing="15" id="nav">
<tr>
<td align="center" colspan="4"><div class="uni"><p>Disciplinary Action</p></div></td>
</tr>

<tr>
<td>Slip No:</td>
<td><select name="cmbslip" id="txt" onchange="showslip()">
<option value="Null"><center>-Select-</center></option>
<%
doconnect();
try{
	stmtfillslip=conn.createStatement();
	rsfillslip=stmtfillslip.executeQuery("select * from vet_tbldisaction");
	while(rsfillslip.next()){
		cslip=rsfillslip.getString("slip_no");
		%>
		<option value="<%=cslip%>"> <%=cslip%>  </option>
...
<td>Date:</td>
<td><input type="date" name="txtdat" id="txt"></td>
</tr>
...
<td><input type="text" name="txtdis" id="txt"></td>
