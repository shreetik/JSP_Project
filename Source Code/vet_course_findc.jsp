...
<%!
Connection conn;
Statement stmtfillccod,stmtfillarray;
ResultSet rscod,rsarray;
String cnames[],sems[],cfees[];
String cod;
int i;
...
try{
	...
	i=0;
	while(rsarray.next()){
		 cnames[i]=rsarray.getString("c_name");
		 sems[i]=rsarray.getString("no_of_sem").trim();
		 cfees[i]=rsarray.getString("c_fee");
...
%>
  ...
  document.f1.txtcname.value=jcnames[k-1];
  document.f1.cmbsem.value=jsems[k-1];
  document.f1.txtcfee.value=jcfees[k-1];
...
<form name="f1" action="vet_course_finds.jsp" method="get">
...
<tr>
<td>Course Code:</td>
<td><select name="cmbcod" id="txt" onchange="showdata()">
<option value="null">--select--</option>
<%
doconnect();
try{
	stmtfillccod=conn.createStatement();
	rscod=stmtfillccod.executeQuery("select * from vet_tblcourse");
	while(rscod.next()){
		cod=rscod.getString("c_code");
		%>
